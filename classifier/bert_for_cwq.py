...
"""
CUDA_VISIBLE_DEVICES=0 python classifier/bert_for_cwq.py
"""
...
def read_data(data):
    ...
    qtypes = [d["compositionality_type"] for d in data]
...
train_data = []
for p in glob("human-anno/cwq/**/*.json"):
    d = read_json(p)
...
dev_data = []
for p in glob("dataset_processed/cwq/test/*.json"):
    d = read_json(p)
    for _d in d:
        dev_data.append(_d)
...
training_args = TrainingArguments(
    output_dir="result-cwq-classification",
    num_train_epochs=30,
...
trainer.train()
model.save_pretrained("result-cwq-classification/best_model")
