...
def save_zero_three_model(model_ema, global_rank, save_dir, zero_stage=0, epoch=None):
    """
    Attention:
        model.layers.xxx.self_attn.rotary_emb.inv_freq 都没有
    note: model.state_dict() != model.named_parameters()
    model.state_dict() v: torch.Tensor
    model.named_parameters() v: torch.nn.parameter.Parameter
    """
    zero_stage_3 = zero_stage == 3
    ...
    if not zero_stage_3:
        ...
        # fix
        for k1, v1 in model_to_save.state_dict().items():
            if k1.endswith(".inv_freq"):
                output_state_dict[k1] = v1.cpu()
        ...
        # sorted
        output_state_dict = {k: output_state_dict[k] for k in sorted(output_state_dict.keys())}
