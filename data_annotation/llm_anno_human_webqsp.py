...
def load_demos():
    demo_ids = []
    demos = []
    # you can add more demos here.
    for n in ["chain_len-01.txt", "chain_len-02.txt"]:
        with open(f"fewshot_demo/webqsp/dialog/{n}", "r") as f:
            demo = f.readlines()
...
def run(qtype="chain_len_1"):
    """
    every type 50 cases.
    save to human-anno/webqsp/[chain_len_1, chain_len_2, manual]/{id}.json
    """
    assert qtype in ["chain_len_1", "chain_len_2", "manual"]

    demo_ids, demos = load_demos()
    tooldesc_demos = TOOL_DESC_FULL_FB + "\n\n" + demos

    # split by qtype
    qtype_idmap = read_json("dataset_processed/webqsp/train_qtype_ids.json")
    # ['chain_len_1', 'chain_len_2', 'manual']
    qtype_idmap = {k: set(v) for k, v in qtype_idmap.items()}

    data = read_json(f"dataset_processed/webqsp/train.json")
    random.seed(123)
    random.shuffle(data)

    skip_ids = []
    bad_num = 0
    paths = glob(f"human-anno/webqsp/{qtype}/*.json")
    for p in paths:
        d = read_json(p)
        if d["done"]:
    ...
    data_tp = [
        d
        for d in data
        if d["id"] in qtype_idmap[qtype] and d["id"] not in skip_ids and d["id"] not in demo_ids
    ]
    if qtype in ["chain_len_1", "chain_len_2"]:
        remain_num = 50 - len(skip_ids) + bad_num
    else:
        remain_num = 10 - len(skip_ids) + bad_num
    ...
    for d in data_tp:
        if remain_num == 0:
            break
        print("-" * 50, d["id"], "-" * 50)
        print("Current:", qtype, "Len:", len(glob(f"human-anno/webqsp/{qtype}/*.json")))
        ...
        chat_anno(
            ...
            save_dir=f"human-anno/webqsp/{qtype}",
            max_round_num=20,
        )
        remain_num -= 1
...
def check_data():
    ...
    """
    qtypes = ["chain_len_1", "chain_len_2", "manual"]
    for qtype in qtypes:
        paths = glob(f"human-anno/webqsp/{qtype}/*.json")
        valid_num = 0
        skip_num = 0
...
if __name__ == "__main__":
    ...
    python llm_anno_human_webqsp.py --qtype chain_len_2
    python llm_anno_human_webqsp.py --qtype manual
    """
    fire.Fire(run)
    # check_data()
